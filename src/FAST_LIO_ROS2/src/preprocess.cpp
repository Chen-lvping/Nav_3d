...
void Preprocess::avia_handler(const livox_ros_driver2::msg::CustomMsg::UniquePtr &msg)
{
  ...
  else
  {
    ...
    // livox_ros_driver2 may interleave MID360 packets by a few hundred
    // microseconds. IMU undistortion walks the cloud in time order, so retain
    // equal-time ordering while enforcing monotonic point offsets. This is
    // the same correction validated by the reference hardware project.
    const auto by_offset_time = [](const PointType& lhs,
                                   const PointType& rhs) {
      return lhs.curvature < rhs.curvature;
    };
    if (!std::is_sorted(pl_surf.points.begin(), pl_surf.points.end(),
                        by_offset_time))
    {
      ...
      if (!warned)
      {
        RCLCPP_WARN(rclcpp::get_logger("fast_lio_preprocess"),
                    "Livox offsets were not monotonic; stable-sorted the "
                    "undistortion input");
        warned = true;
      }
    }
