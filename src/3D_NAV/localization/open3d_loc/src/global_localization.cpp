                                                                                             ...
                                                                                             nh_private_(nh_private)
{
    ...
    sub_baselink2odom_ = nh_.subscribe("/Odometry", 50, &GloabalLocalization::CallbackBaselink2Odom, this);
    // sub_scan_cur_ = nh_.subscribe("/cloud_registered_1", 50, &GloabalLocalization::CallbackScan, this);
    sub_scan_cur_ = nh_.subscribe("/cloud_registered", 50, &GloabalLocalization::CallbackScan, this);
...
{
    ROS_INFO("wait for Odometry");
    // ros::topic::waitForMessage<nav_msgs::Odometry>("/Odometry_loc");
    ros::topic::waitForMessage<nav_msgs::Odometry>("/Odometry");
    ROS_INFO("wait for cloud_registered");
    // ros::topic::waitForMessage<sensor_msgs::PointCloud2>("/cloud_registered_1");
    ros::topic::waitForMessage<sensor_msgs::PointCloud2>("/cloud_registered");
    // initialize
    /****初始化定位****/
