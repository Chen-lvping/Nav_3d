...
Set ``` pcd_save_enable ``` in launchfile to ``` 1 ```. In this workspace, MID360 mapping saves the accumulated global-frame scan to ```/home/arts-inpector/ART_3dnav/src/data/point_cloud/scans.pcd``` after FAST-LIO is terminated. ```pcl_viewer scans.pcd``` can visualize the point clouds.
