...
bool TraversableExtractor::extractTraversableAreas() {
    ...
    // Save seed points for debugging
    std::string seed_points_file = params_.debug_output_dir + "/seed_points.pcd";
    if (Utils::savePointCloud(seed_points, seed_points_file)) {
        std::cout << "Saved " << seed_points->size() << " seed points to: " << seed_points_file << std::endl;
    }
...
bool TraversableExtractor::performRegionGrowingWithSeeds(const pcl::PointCloud<pcl::PointXYZI>::Ptr& seed_points) {
    ...
    for (const auto& cluster : clusters) {
        ...
        // Only keep clusters that contain seed points and meet size requirements
        if (contains_seed && cluster.indices.size() >= static_cast<size_t>(params_.min_region_size)) {
            valid_clusters.push_back(cluster);
        }
    }
    
    std::cout << "Found " << valid_clusters.size() << " valid clusters containing seed points" << std::endl;
