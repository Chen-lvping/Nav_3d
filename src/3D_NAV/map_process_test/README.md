...
- 调试输出：种子点会尝试保存到 `/home/arts/Galilio/map_process_test_ws/src/map_process/output/seed_points.pcd`（路径在 `src/traversable_extractor.cpp` 中硬编码，如与实际路径不符需自行修改）。
...
- 种子点：默认硬编码保存到 `/home/arts/Galilio/map_process_test_ws/src/map_process/output/seed_points.pcd`。
